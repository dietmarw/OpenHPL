...
equation
  ...
  connect(f_grid_int, speedReference.u1);
  connect(speedBias.y, speedReference.u2);
  connect(speedReference.y, speedRefNorm.u);
  connect(f, speedMeasNorm.u);
  connect(speedRefNorm.y, piSpeed.u_s);
  connect(speedMeasNorm.y, piSpeed.u_m);
  connect(P_ref, powerRefNorm.u);
  connect(P_meas, powerMeasNorm.u);
  connect(f_grid_int, droopError.u1);
  connect(f, droopError.u2);
  connect(droopError.y, droopGain.u);
  connect(powerRefNorm.y, powerReference.u1);
  connect(droopGain.y, powerReference.u2);
  connect(zero.y, powerReference.u3);
  connect(powerMeasNorm.y, piPower.u_m);
  connect(powerReference.y, piPower.u_s);
  connect(piPower.y, powerModeSwitch.u1);
  connect(piSpeed.y, powerModeSwitch.u3);
  connect(powerMode, powerModeSwitch.u2);
  connect(powerModeSwitch.y, closedLoopSwitch.u1);
  connect(zero.y, closedLoopSwitch.u3);
  connect(closedLoop, closedLoopSwitch.u2);
  connect(closedLoopSwitch.y, startSwitch.u3);
  connect(startOpening.y, startSwitch.u1);
  connect(startingMode, startSwitch.u2);
  Y_cmd = startSwitch.y;
